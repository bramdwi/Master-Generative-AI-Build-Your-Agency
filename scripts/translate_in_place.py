    ...
    for line in lines:
        ...
        if stripped.startswith("#"):
            ...
            if match:
                ...
                out_lines.append(f"{prefix}{translate_text_block(title)}\n")
                continue
        ...
        if re.match(r'^(\*|-|\d+\.)\s+', stripped):
            ...
            if match:
                ...
                out_lines.append(f"{bullet} {translate_text_block(body)}\n")
                continue
        ...
        if stripped.startswith(">"):
            ...
            if match:
                ...
                out_lines.append(f"{prefix}{translate_text_block(quote)}\n")
                continue

        # Normal text line
        out_lines.append(f"{translate_text_block(stripped)}\n")
...
if __name__ == "__main__":
    ...
    # Root markdown files
    root_mds = ["README.md", "ROADMAP.md", "LESSON_TEMPLATE.md", "CONTRIBUTING.md"]
    for rfile in root_mds:
        fp = os.path.join(project_root, rfile)
        if os.path.isfile(fp):
            translate_file_in_place(fp)

    # All tracks markdown files
    ...
    print("\n[SUCCESS] Completed in-place translation of all primary .md files!", flush=True)
